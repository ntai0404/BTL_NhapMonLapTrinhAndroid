...
public class AdminProductAdapter extends RecyclerView.Adapter<AdminProductAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.binding.productTitle.setText(currentItem.getTitle());

        java.text.NumberFormat currencyFormatter = java.text.NumberFormat.getCurrencyInstance(new java.util.Locale("vi", "VN"));
        holder.binding.productPrice.setText(currencyFormatter.format(currentItem.getPrice()));

        if (currentItem.getPicUrl() != null && !currentItem.getPicUrl().isEmpty()) {
            Glide.with(context).load(currentItem.getPicUrl().get(0)).into(holder.binding.productImage);
        }

        holder.binding.editBtn.setOnClickListener(v -> clickListener.onEditClick(currentItem));
        holder.binding.deleteBtn.setOnClickListener(v -> clickListener.onDeleteClick(currentItem, holder.getAdapterPosition()));
    }
